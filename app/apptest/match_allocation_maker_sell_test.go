...
func Test_Maker_Sell_1a(t *testing.T) {
	...
	*/
	...
	assert.Equal(true, RoundOrderExists("BTC-000_BNB", oid, 1, 12e8))
	...
	assert.NoError(err)
	assert.Equal(true, OrderExists("BTC-000_BNB", oid))
	assert.Equal(true, RoundOrderExists("BTC-000_BNB", oid, 1, 10e8))
	...
	assert.NoError(err)
	assert.Equal(true, OrderExists("BTC-000_BNB", oid))
	assert.Equal(true, RoundOrderExists("BTC-000_BNB", oid, 1, 9e8))
	...
	assert.NoError(err)
	assert.Equal(true, OrderExists("BTC-000_BNB", oid))
	assert.Equal(true, RoundOrderExists("BTC-000_BNB", oid, 1, 8e8))
	...
	assert.NoError(err)
	assert.Equal(true, OrderExists("BTC-000_BNB", oid))
	assert.Equal(true, RoundOrderExists("BTC-000_BNB", oid, 2, 8e8))
	...
	assert.NoError(err)
	assert.Equal(true, OrderExists("BTC-000_BNB", oid))
	assert.Equal(true, RoundOrderExists("BTC-000_BNB", oid, 2, 6e8))
...
func Test_Maker_Sell_2c(t *testing.T) {
	...
	*/
	...
	orders := GetRoundPlOrders("BTC-000_BNB", 1, 10e8)
	sum := int64(0)
	for _, order := range orders {
		sum += order.Quantity - order.CumQty
	}
	assert.Equal(int64(75e8), sum)
...
func Test_Maker_Sell_5(t *testing.T) {
	...
	*/
	...
	oid = GetOrderId(addr0, 3, ctx)
	msgB1 := order.NewNewOrderMsg(addr0, oid, 1, "BTC-000_BNB", 6e8, 10e8)
	_, err = testClient.DeliverTxSync(msgB1, testApp.Codec)
	assert.NoError(err)

	oid = GetOrderId(addr0, 4, ctx)
	msgB2 := order.NewNewOrderMsg(addr0, oid, 1, "BTC-000_BNB", 5e8, 10e8)
	...
	msgC1 := order.NewCancelOrderMsg(addr1, "BTC-000_BNB", msgS.Id)
	_, err = testClient.DeliverTxSync(msgC1, testApp.Codec)
	assert.NoError(err)

	msgC2 := order.NewCancelOrderMsg(addr0, "BTC-000_BNB", msgB1.Id)
	_, err = testClient.DeliverTxSync(msgC2, testApp.Codec)
	assert.NoError(err)

	msgC3 := order.NewCancelOrderMsg(addr0, "BTC-000_BNB", msgB2.Id)
	...
	assert.Equal(int64(100000e8), GetAvail(ctx, addr0, "BTC-000"))
	assert.Equal(int64(99630.9996e8), GetAvail(ctx, addr0, "BNB"))
	assert.Equal(int64(369e8), GetLocked(ctx, addr0, "BNB"))
	...
	assert.Equal(int64(100032e8), GetAvail(ctx, addr0, "BTC-000"))
	assert.Equal(int64(99695.8791e8), GetAvail(ctx, addr0, "BNB"))
...
func Test_Maker_Sell_8(t *testing.T) {
	...
	*/
	...
	orders := GetRoundPlOrders("BTC-000_BNB", 2, 8e8)
	sum := int64(0)
	for _, order := range orders {
		sum += order.Quantity - order.CumQty
	}
	assert.Equal(int64(20e8), sum)
...
func Test_Maker_Sell_9a(t *testing.T) {
	...
	*/
	...
	assert.Equal(0, len(buys))
	assert.Equal(1, len(sells))
	assert.Equal(utils.Fixed8(3e8), sells[0].qty)
	...
	assert.Equal(int64(17e8), sum)

	assert.Equal(int64(100000e8), GetAvail(ctx, addr0, "BTC-000"))
	assert.Equal(int64(99810e8), GetAvail(ctx, addr0, "BNB"))
	assert.Equal(int64(190e8), GetLocked(ctx, addr0, "BNB"))
	...
	assert.Equal(int64(99823.912e8), GetAvail(ctx, addr0, "BNB"))
	assert.Equal(int64(0), GetLocked(ctx, addr0, "BNB"))
	assert.Equal(int64(99997e8), GetAvail(ctx, addr1, "BTC-000"))
	assert.Equal(int64(100023.988e8), GetAvail(ctx, addr1, "BNB"))
	assert.Equal(int64(0), GetLocked(ctx, addr1, "BTC-000"))
	assert.Equal(int64(99978e8), GetAvail(ctx, addr2, "BTC-000"))
	assert.Equal(int64(100151.9240e8), GetAvail(ctx, addr2, "BNB"))
...
func Test_Maker_Sell_9b(t *testing.T) {
	...
	*/
	...
	orders := GetRoundPlOrders("BTC-000_BNB", 2, 7e8)
	sum := int64(0)
	for _, order := range orders {
		sum += order.Quantity - order.CumQty
	}
	assert.Equal(int64(55e8), sum)
...
func Test_Maker_Sell_11_new(t *testing.T) {
	...
	msgS1 := order.NewNewOrderMsg(addr0, oidS1, 2, "BTC-000_BNB", 15e8, 10e8)
	_, err := testClient.DeliverTxSync(msgS1, testApp.Codec)
	assert.NoError(err)

	oidS2 := GetOrderId(addr0, 1, ctx)
	msgS2 := order.NewNewOrderMsg(addr0, oidS2, 2, "BTC-000_BNB", 15e8, 5e8)
	...
	msgS3 := order.NewNewOrderMsg(addr0, oidS3, 2, "BTC-000_BNB", 15e8, 6e8)
	_, err = testClient.DeliverTxSync(msgS3, testApp.Codec)
	assert.NoError(err)

	oidB := GetOrderId(addr1, 0, ctx)
	msgB := order.NewNewOrderMsg(addr1, oidB, 1, "BTC-000_BNB", 15e8, 12e8)
	_, err = testClient.DeliverTxSync(msgB, testApp.Codec)
	assert.NoError(err)

	msgC := order.NewCancelOrderMsg(addr0, "BTC-000_BNB", msgS2.Id)
	_, err = testClient.DeliverTxSync(msgC, testApp.Codec)
	assert.NoError(err)
	...
	orders := GetRoundPlOrders("BTC-000_BNB", 2, 15e8)
	sum := int64(0)
	for _, order := range orders {
		sum += order.Quantity - order.CumQty
	}
	assert.Equal(int64(6e8), sum)
	// match has not taken place, so no bnb balance, fee charged in btc-000
	// 0.001 bnb -> 0.001/10 = 0.0001
	...
	assert.Equal(int64(99820e8), GetAvail(ctx, addr1, "BNB"))
	assert.Equal(int64(180e8), GetLocked(ctx, addr1, "BNB"))
	...
	assert.Equal(int64(99983.9999e8), GetAvail(ctx, addr0, "BTC-000"))
	assert.Equal(int64(179.9100e8), GetAvail(ctx, addr0, "BNB"))
	assert.Equal(int64(4e8), GetLocked(ctx, addr0, "BTC-000"))
	assert.Equal(int64(100012e8), GetAvail(ctx, addr1, "BTC-000"))
	assert.Equal(int64(99819.9100e8), GetAvail(ctx, addr1, "BNB"))
	assert.Equal(int64(0), GetLocked(ctx, addr1, "BNB"))
}
