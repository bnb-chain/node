...
func SubmitSCParamChangeProposalCmd(cdc *codec.Codec) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			scParamsBz, err := cdc.MarshalJSON(scParams)
			if err != nil {
				return err
			}
...
func ShowSideChainParamsCmd(cdc *amino.Codec) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "side-params",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			bz, err := cliCtx.Query(fmt.Sprintf("%s/sideParams", paramHub.AbciQueryPrefix), data)
			if err != nil {
				return err
			}
