...
func UnmarshalFixedText(typname string, input, out []byte) error {
	...
	for _, b := range raw {
		if HexDecodeNibble(b) == BadNibble {
			return ErrSyntax
		}
	}
	hex.Decode(out, raw)
...
func (b Uint64) String() string {
	return HexEncodeUint64(uint64(b))
}
...
func (b *Uint) UnmarshalText(input []byte) error {
	...
	if u64 > Uint64(^uint(0)) || err == ErrUint64Range {
		return errors.New(fmt.Sprintf("hex number > %d bits", UintBits))
	} else if err != nil {
		return err
	}
	*b = Uint(u64)
