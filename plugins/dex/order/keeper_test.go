...
func TestKeeper_MatchFailure(t *testing.T) {
	...
	tradingPair := dextypes.NewTradingPair("XYZ-000000", "BNB", 1e8)
	tradingPair.LotSize = -10000000 // negative LotSize should never happen
	keeper.PairMapper.AddTradingPair(ctx, tradingPair)
	keeper.AddEngine(tradingPair)

	msg := NewNewOrderMsg(accAdd, "123456", Side.BUY, "XYZ-000000_BNB", 99000, 3000000)
	ord := OrderInfo{msg, 42, 0, 42, 0, 0, ""}
	keeper.AddOrder(ord, false)
	msg = NewNewOrderMsg(accAdd, "123457", Side.BUY, "XYZ-000000_BNB", 99000, 1000000)
	ord = OrderInfo{msg, 42, 0, 42, 0, 0, ""}
	keeper.AddOrder(ord, false)
	msg = NewNewOrderMsg(accAdd, "123458", Side.BUY, "XYZ-000000_BNB", 99000, 5000000)
	ord = OrderInfo{msg, 42, 0, 42, 0, 0, ""}
	keeper.AddOrder(ord, false)
	msg = NewNewOrderMsg(accAdd, "123459", Side.SELL, "XYZ-000000_BNB", 98000, 1000000)
	ord = OrderInfo{msg, 42, 0, 42, 0, 0, ""}
	keeper.AddOrder(ord, false)
	msg = NewNewOrderMsg(accAdd, "123460", Side.SELL, "XYZ-000000_BNB", 97000, 5000000)
	ord = OrderInfo{msg, 42, 0, 42, 0, 0, ""}
	keeper.AddOrder(ord, false)
	msg = NewNewOrderMsg(accAdd, "123461", Side.SELL, "XYZ-000000_BNB", 95000, 5000000)
	ord = OrderInfo{msg, 42, 0, 42, 0, 0, ""}
	keeper.AddOrder(ord, false)
	msg = NewNewOrderMsg(accAdd, "123462", Side.BUY, "XYZ-000000_BNB", 99000, 15000000)
	ord = OrderInfo{msg, 42, 0, 42, 0, 0, ""}
	keeper.AddOrder(ord, false)
...
func TestKeeper_SnapShotOrderBook(t *testing.T) {
	...
	msg := NewNewOrderMsg(accAdd, "123456", Side.BUY, "XYZ-000000_BNB", 102000, 3000000)
	keeper.AddOrder(OrderInfo{msg, 42, 84, 42, 84, 0, ""}, false)
	msg = NewNewOrderMsg(accAdd, "123457", Side.BUY, "XYZ-000000_BNB", 101000, 1000000)
	keeper.AddOrder(OrderInfo{msg, 42, 84, 42, 84, 0, ""}, false)
	msg = NewNewOrderMsg(accAdd, "123458", Side.BUY, "XYZ-000000_BNB", 99000, 5000000)
	keeper.AddOrder(OrderInfo{msg, 42, 84, 42, 84, 0, ""}, false)
	msg = NewNewOrderMsg(accAdd, "123459", Side.SELL, "XYZ-000000_BNB", 98000, 1000000)
	...
	msg = NewNewOrderMsg(accAdd, "123461", Side.SELL, "XYZ-000000_BNB", 95000, 5000000)
	keeper.AddOrder(OrderInfo{msg, 42, 84, 42, 84, 0, ""}, false)
	msg = NewNewOrderMsg(accAdd, "123462", Side.BUY, "XYZ-000000_BNB", 96000, 1500000)
	keeper.AddOrder(OrderInfo{msg, 42, 84, 42, 84, 0, ""}, false)
	assert.Equal(1, len(keeper.allOrders))
	assert.Equal(7, len(keeper.allOrders["XYZ-000000_BNB"]))
	...
	assert.Equal(7, len(keeper2.allOrders["XYZ-000000_BNB"]))
	o123459 := keeper2.allOrders["XYZ-000000_BNB"]["123459"]
	...
	assert.Equal(int64(43), h)
	buys, sells := keeper2.engines["XYZ-000000_BNB"].Book.GetAllLevels()
...
func TestKeeper_SnapShotAndLoadAfterMatch(t *testing.T) {
	...
	msg123456 := NewNewOrderMsg(accAdd, "123456", Side.BUY, "XYZ-000000_BNB", 102000, 3000000)
	info123456 := OrderInfo{msg123456, 42, 0, 42, 0, 0, ""}
	keeper.AddOrder(info123456, false)
	msg123457 := NewNewOrderMsg(accAdd, "123457", Side.BUY, "XYZ-000000_BNB", 10000, 1000000)
	info123457 := OrderInfo{msg123457, 42, 0, 42, 0, 0, ""}
	keeper.AddOrder(info123457, false)
	msg := NewNewOrderMsg(accAdd, "123458", Side.SELL, "XYZ-000000_BNB", 100000, 2000000)
	keeper.AddOrder(OrderInfo{msg, 42, 0, 42, 0, 0, ""}, false)
	assert.Equal(1, len(keeper.allOrders))
	assert.Equal(3, len(keeper.allOrders["XYZ-000000_BNB"]))
	assert.Equal(1, len(keeper.engines))
	...
	keeper2 := MakeKeeper(cdc)
	h, err := keeper2.LoadOrderBookSnapshot(ctx, 10)
	assert.Equal(2, len(keeper2.allOrders["XYZ-000000_BNB"]))
	assert.Equal(int64(102000), keeper2.allOrders["XYZ-000000_BNB"]["123456"].Price)
	assert.Equal(int64(2000000), keeper2.allOrders["XYZ-000000_BNB"]["123456"].CumQty)
	assert.Equal(int64(10000), keeper2.allOrders["XYZ-000000_BNB"]["123457"].Price)
	assert.Equal(int64(0), keeper2.allOrders["XYZ-000000_BNB"]["123457"].CumQty)
	...
	assert.Equal(1, len(keeper2.engines))
	assert.Equal(int64(102000), keeper2.engines["XYZ-000000_BNB"].LastTradePrice)
...
func TestKeeper_SnapShotOrderBookEmpty(t *testing.T) {
	...
	msg := NewNewOrderMsg(accAdd, "123456", Side.BUY, "XYZ-000000_BNB", 102000, 300000)
	keeper.AddOrder(OrderInfo{msg, 42, 0, 42, 0, 0, ""}, false)
	keeper.RemoveOrder(msg.Id, msg.Symbol, nil)
	buys, sells := keeper.engines["XYZ-000000_BNB"].Book.GetAllLevels()
	...
	h, err := keeper2.LoadOrderBookSnapshot(ctx, 10)
	assert.Equal(int64(43), h)
	assert.Equal(0, len(keeper2.allOrders["XYZ-000000_BNB"]))
	buys, sells = keeper2.engines["XYZ-000000_BNB"].Book.GetAllLevels()
	assert.Equal(0, len(buys))
	assert.Equal(0, len(sells))
}
...
func TestKeeper_LoadOrderBookSnapshot(t *testing.T) {
	...
	keeper.PairMapper.AddTradingPair(ctx, dextypes.NewTradingPair("XYZ-000000", "BNB", 1e8))
...
func GenerateBlocksAndSave(storedb db.DB, cdc *wire.Codec) *bc.BlockStore {
	...
	msgs01 := []sdk.Msg{NewNewOrderMsg(buyerAdd, "123456", Side.BUY, "XYZ-000000_BNB", 102000, 3000000)}
	txs[0] = MakeTxFromMsg(msgs01, int64(100), int64(9001), buyerPrivKey)
	msgs02 := []sdk.Msg{NewNewOrderMsg(buyerAdd, "123457", Side.BUY, "XYZ-000000_BNB", 101000, 1000000)}
	txs[1] = MakeTxFromMsg(msgs02, int64(100), int64(9002), buyerPrivKey)
	msgs03 := []sdk.Msg{NewNewOrderMsg(sellerAdd, "123459", Side.SELL, "XYZ-000000_BNB", 98000, 1000000)}
	txs[2] = MakeTxFromMsg(msgs03, int64(1001), int64(7001), sellerPrivKey)
	msgs04 := []sdk.Msg{NewNewOrderMsg(buyerAdd, "123458", Side.BUY, "XYZ-000000_BNB", 99000, 5000000)}
	txs[3] = MakeTxFromMsg(msgs04, int64(100), int64(9003), buyerPrivKey)
	msgs05 := []sdk.Msg{NewNewOrderMsg(sellerAdd, "123460", Side.SELL, "XYZ-000000_BNB", 97000, 5000000)}
	txs[4] = MakeTxFromMsg(msgs05, int64(1001), int64(7002), sellerPrivKey)
	msgs06 := []sdk.Msg{NewNewOrderMsg(sellerAdd, "123461", Side.SELL, "XYZ-000000_BNB", 95000, 5000000)}
	txs[5] = MakeTxFromMsg(msgs06, int64(1001), int64(7003), sellerPrivKey)
	msgs07 := []sdk.Msg{NewNewOrderMsg(buyerAdd, "123462", Side.BUY, "XYZ-000000_BNB", 96000, 1500000)}
	...
	height++
	msgs11 := []sdk.Msg{NewNewOrderMsg(buyerAdd, "123463", Side.BUY, "XYZ-000000_BNB", 96000, 2500000)}
	msgs12 := []sdk.Msg{NewNewOrderMsg(buyerAdd, "123464", Side.BUY, "XYZ-000000_BNB", 97000, 1500000)}
	msgs13 := []sdk.Msg{NewNewOrderMsg(sellerAdd, "123465", Side.SELL, "XYZ-000000_BNB", 107000, 1500000)}
	msgs14 := []sdk.Msg{NewCancelOrderMsg(buyerAdd, "XYZ-000000_BNB", "123466", "123462")}
	msgs15 := []sdk.Msg{NewCancelOrderMsg(sellerAdd, "XYZ-000000_BNB", "123467", "123465")}
...
func TestKeeper_ExpireOrders(t *testing.T) {
	...
	addr := acc.GetAddress()
	keeper.AddEngine(dextypes.NewTradingPair("ABC-000000", "BNB", 1e6))
	keeper.AddEngine(dextypes.NewTradingPair("XYZ-000000", "BNB", 1e6))
	keeper.AddOrder(OrderInfo{NewNewOrderMsg(addr, "1", Side.BUY, "ABC-000000_BNB", 1e6, 1e6), 10000, 0, 10000, 0, 0, ""}, false)
	keeper.AddOrder(OrderInfo{NewNewOrderMsg(addr, "2", Side.BUY, "ABC-000000_BNB", 2e6, 2e6), 10000, 0, 10000, 0, 0, ""}, false)
	keeper.AddOrder(OrderInfo{NewNewOrderMsg(addr, "3", Side.BUY, "XYZ-000000_BNB", 1e6, 2e6), 10000, 0, 10000, 0, 0, ""}, false)
	keeper.AddOrder(OrderInfo{NewNewOrderMsg(addr, "4", Side.SELL, "ABC-000000_BNB", 1e6, 1e8), 10000, 0, 10000, 0, 0, ""}, false)
	keeper.AddOrder(OrderInfo{NewNewOrderMsg(addr, "5", Side.SELL, "ABC-000000_BNB", 2e6, 2e8), 15000, 0, 15000, 0, 0, ""}, false)
	keeper.AddOrder(OrderInfo{NewNewOrderMsg(addr, "6", Side.BUY, "XYZ-000000_BNB", 2e6, 2e6), 20000, 0, 20000, 0, 0, ""}, false)
	acc.(types.NamedAccount).SetLockedCoins(sdk.Coins{
		sdk.NewCoin("ABC-000000", 3e8),
	...
	require.Equal(t, sdk.Coins{
		sdk.NewCoin("ABC-000000", 2e8),
		sdk.NewCoin("BNB", 4e4),
	}.Sort(), acc.(types.NamedAccount).GetLockedCoins())
	require.Equal(t, sdk.Coins{
		sdk.NewCoin("ABC-000000", 9e7),
		sdk.NewCoin("BNB", 1e4),
	}.Sort(), acc.GetCoins())
	fees.Pool.Clear()
}
