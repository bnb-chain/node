...
func (kp *Keeper) matchAndDistributeTradesForSymbol(symbol string, distributeTrade bool,
	tradeOuts []chan Transfer, logger log.Logger) {
	...
	if engine.Match() {
		logger.Info("Match finish:", "symbol", symbol, "lastTradePrice", engine.LastTradePrice)
		if distributeTrade {
		...
		logger.Info("Drop filled orders", "total", n)
	...
	for _, id := range iocIDs {
		if msg, ok := kp.allOrders[id]; ok {
			if ord, err := kp.RemoveOrder(msg.Id, msg.Symbol, msg.Side, msg.Price); err == nil {
				logger.Info("Removed unclosed IOC order", "ordID", msg.Id)
				if !distributeTrade {
					continue
				}
...
func (kp *Keeper) matchAndDistributeTrades(distributeTrade bool, logger log.Logger) []chan Transfer {
	...
	matchWorker := func() {
		...
		for symbol := range symbolCh {
			kp.matchAndDistributeTradesForSymbol(symbol, distributeTrade, tradeOuts, logger)
		}
	}
...
func (kp *Keeper) calculateOrderFee(ctx sdk.Context, account auth.Account, tran Transfer) types.Fee {
	...
	if tran.inAsset == types.NativeToken {
		...
		if account.GetCoins().AmountOf(types.NativeToken).Int64() >= feeByNativeToken {
			...
			logger := ctx.Logger()
			logger.Info("Not enough native token to pay trade fee", "feeToken", feeToken)
		}
	}
...
func (kp *Keeper) MatchAll(logger log.Logger) (code sdk.CodeType, err error) {
	tradeOuts := kp.matchAndDistributeTrades(false, logger) //only match
	if tradeOuts == nil {
		// TODO: logging
...
func (kp *Keeper) MatchAndAllocateAll(ctx sdk.Context, accountMapper auth.AccountMapper, logger log.Logger,
	postAllocateHandler func(tran Transfer)) (newCtx sdk.Context, code sdk.CodeType, err error) {
	var wg sync.WaitGroup
	tradeOuts := kp.matchAndDistributeTrades(true, logger)
	if tradeOuts == nil {
		// TODO: logging
...
func (kp *Keeper) GetBreatheBlockHeight(timeNow time.Time, kvStore sdk.KVStore,
	daysBack int, logger log.Logger) int64 {
	bz := []byte(nil)
...
func (kp *Keeper) InitGenesis(ctx sdk.Context, genesis TradingGenesis) {
	ctx.Logger().Info("Initializing Fees from Genesis")
