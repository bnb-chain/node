...
func TestHashTimerLockTransferMsg(t *testing.T) {
	...
	for i, tc := range tests {
		recipientOtherChain, _ := hex.DecodeString(tc.ToOnOtherChain)
		randomNumberHash, _ := hex.DecodeString(tc.RandomNumberHash)
		msg := NewHashTimerLockTransferMsg(tc.From, tc.To, recipientOtherChain, randomNumberHash, tc.Timestamp, tc.OutAmount, tc.InAmountOtherChain, tc.HeightSpan)
