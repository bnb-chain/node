...
func Routes(tokenMapper store.Mapper, accKeeper auth.AccountKeeper, keeper bank.Keeper,
	timeLockKeeper timelock.Keeper, swapKeeper swap.Keeper, crossChainKeeper cross_chain.Keeper) map[string]sdk.Handler {
	...
	routes[cross_chain.CrossChainRoute] = cross_chain.NewHandler(crossChainKeeper)
	return routes
}
